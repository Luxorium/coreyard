...
def primary_row(product: RenderedProduct) -> dict[str, str]:
...
def rows_for(product: RenderedProduct) -> list[dict[str, str]]:
    """Full row set for one rendered product: primary row + one row per extra image."""
    rows = [primary_row(product)]
...
def part_to_rows(
    ...
    return rows_for(render(part, image_urls, store))
