...
| `QA-01` | Continuous gates | IN PROGRESS | Offline suite: 1115 tests, no `.env`, credentials or network, on Python 3.10-3.13 in CI. CI now also checks the functionality inventory, tracked-file whitespace and built-artifact version. Example config validation added for `schema.example.json` (`tests/test_command_inventory.py`) and `store.example.json` (`tests/test_store_file.py`). Outstanding: installed-artifact demo and dependency-metadata jobs. |
