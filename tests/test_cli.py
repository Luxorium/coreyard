...
class Launcher(unittest.TestCase):
    ...
    def test_an_installed_launcher_matches_the_installer(self):
        """The generated launcher must be the installer's, apart from the one substitution.

        `install.sh` writes the heredoc verbatim and then replaces a single token with this
        installation's data home. Comparing the two with that token put back keeps the test
        doing its real job — catching a launcher that has drifted from the script which
        writes it — while allowing the one difference that is supposed to be there.
        """
        launcher = REPO_ROOT / "bin" / "coreyard"
        ...
        marker = "@DATA_HOME@"
        self.assertIn(marker, from_installer, "the installer no longer substitutes a home")
        self.assertNotIn(marker, installed, "install.sh left the placeholder unsubstituted")
        prefix, _, suffix = from_installer.partition(marker)
        self.assertTrue(installed.startswith(prefix), "the launcher has drifted before the home")
        self.assertTrue(installed.endswith(suffix), "the launcher has drifted after the home")
        home = installed[len(prefix):len(installed) - len(suffix)]
        self.assertTrue(home.strip(), "no data home was written into the launcher")
