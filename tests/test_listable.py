...
class ResearchedPriceGate(unittest.TestCase):
    """The other half of "listable": a part with no researched price is not published.

    Same shape as the photo policy above, and for the same reason — sync and reconciliation
    have to answer this identically or they will publish and archive the same part forever.
    """

    KEYS = ("STORE_REQUIRE_RESEARCHED_PRICE", "STORE_CATALOG_OVERRIDES_FILE")

    def setUp(self):
        before = {k: os.environ.get(k) for k in self.KEYS}
    ...
    def _overrides(self, body: str) -> str:
        import tempfile
    ...
    def test_off_by_default_so_an_upgrade_publishes_what_it_always_did(self):
        self.assertFalse(inventory.researched_prices_required())
        self.assertIsNone(inventory.researched_r_numbers())

    def test_on_it_lists_only_the_parts_carrying_a_researched_price(self):
        os.environ["STORE_REQUIRE_RESEARCHED_PRICE"] = "true"
        os.environ["STORE_CATALOG_OVERRIDES_FILE"] = self._overrides(
            '{"version": 1, "parts": {"51": {"price": "49.99"}, '
            '"52": {"title": "Titled but unpriced"}}}')
        self.assertEqual({"51"}, inventory.researched_r_numbers())

    def test_nothing_researched_yet_is_not_the_same_as_no_gate(self):
        # None means "this site does not gate on price"; an empty set means "it does, and
        # nothing qualifies". Collapsing them would unlist the whole catalogue.
        os.environ["STORE_REQUIRE_RESEARCHED_PRICE"] = "true"
        os.environ["STORE_CATALOG_OVERRIDES_FILE"] = self._overrides(
            '{"version": 1, "parts": {}}')
        self.assertEqual(set(), inventory.researched_r_numbers())
        self.assertIsNotNone(inventory.researched_r_numbers())
